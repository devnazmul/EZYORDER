...
interface ILineChartPointerLabelProps {
  item?: {
    value?: number;
    label?: string;
    fullLabel?: string;
...
function LineChartPointerLabel({
  item,
  isAmount,
  currencySymbol,
}: Readonly<ILineChartPointerLabelProps>) {
  if (!item) return null;
  ...
  return (
    ...
    >
      {!isAnimated || isInView ? (
        ...
        <View style={{ height: chartHeight }} className="w-full" />
      )}
    </View>
  );
}
