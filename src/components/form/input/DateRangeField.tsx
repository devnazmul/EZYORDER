// 1. React / React Native
import React, { useState } from "react";
import { Text, TouchableOpacity, View } from "react-native";

// 2. Expo / Navigation
import { MaterialIcons } from "@expo/vector-icons";

// 3. External libraries
import dayjs from "dayjs";
import { Controller, useFormContext } from "react-hook-form";

// 4. Shared components
import DatePickerModal from "@/components/reuseable/DatePickerModal";
import InputError from "./InputError";
import InputLabel from "./InputLabel";

// 7. Constants / utils
import { COLORS } from "@/constants/colors";
import { getResponsiveFontSize, WP } from "@/utils/getResponsiveSizes";
import { handleFieldSideEffects } from "./handleFieldSideEffects";

export interface IDateRangeFieldProps {
  readonly name: string;
  readonly label?: string;
  readonly startLabel?: string;
  readonly endLabel?: string;
  readonly className?: string;
  readonly formatDateLabel?: (dateStr: string) => string;
  readonly onFieldChange?: (
    val: unknown,
    formValues: Record<string, unknown>,
  ) => Record<string, unknown> | void;
}

// Helper sub-component for date picker trigger item
interface IDateItemButtonProps {
  label: string;
  formattedDate: string;
  onPress: () => void;
}

function DateItemButton({
  label,
  formattedDate,
  onPress,
}: Readonly<IDateItemButtonProps>) {
  return (
    <TouchableOpacity
      onPress={onPress}
      activeOpacity={0.8}
      accessibilityRole="button"
      accessibilityLabel={`${label}: ${formattedDate}`}
      style={{ padding: WP("2.5%") }}
      className="flex-1 flex-row items-center justify-between bg-base-100 border border-base-200 rounded-xl"
    >
      <View>
        <Text
          style={{ fontSize: getResponsiveFontSize("xs") - 1 }}
          className="font-semibold text-accent capitalize"
        >
          {label}
        </Text>
        <Text
          style={{ fontSize: getResponsiveFontSize("xs") }}
          className="font-semibold text-neutral mt-0.5"
        >
          {formattedDate}
        </Text>
      </View>
      <MaterialIcons
        name="calendar-today"
        size={WP("4.5%")}
        color={COLORS.primary}
      />
    </TouchableOpacity>
  );
}

export default function DateRangeField({
  name,
  label,
  startLabel = "Start Date",
  endLabel = "End Date",
  className = "",
  formatDateLabel = (dateStr) =>
    dateStr && dayjs(dateStr).isValid()
      ? dayjs(dateStr).format("MMM D, YYYY")
      : dateStr || "Select Date",
  onFieldChange,
}: Readonly<IDateRangeFieldProps>) {
  const [activePickerType, setActivePickerType] = useState<
    "start" | "end" | null
  >(null);
  const { control, getValues, setValue } = useFormContext();

  return (
    <Controller
      control={control}
      name={name}
      render={({ field: { onChange, value }, fieldState: { error } }) => {
        const range = (value as { start?: string; end?: string }) || {
          start: "",
          end: "",
        };

        const handleSelectDate = (dateStr: string) => {
          if (!activePickerType) return;
          const newRange = {
            ...range,
            [activePickerType]: dateStr,
          };

          onChange(newRange);
          setActivePickerType(null);
          handleFieldSideEffects(
            name,
            newRange,
            getValues,
            setValue,
            onFieldChange,
          );
        };

        const selectedDateForPicker = activePickerType
          ? range[activePickerType]
          : undefined;

        return (
          <View className={`w-full ${className}`}>
            <InputLabel label={label} className="capitalize mb-3" />
            <View className="flex-row items-center gap-3">
              <DateItemButton
                label={startLabel}
                formattedDate={formatDateLabel(range.start || "")}
                onPress={() => setActivePickerType("start")}
              />
              <DateItemButton
                label={endLabel}
                formattedDate={formatDateLabel(range.end || "")}
                onPress={() => setActivePickerType("end")}
              />
            </View>

            {Boolean(error?.message) && (
              <InputError errorMessage={error?.message || ""} />
            )}

            {Boolean(activePickerType) && (
              <DatePickerModal
                visible={true}
                onClose={() => setActivePickerType(null)}
                selectedDate={selectedDateForPicker}
                onSelectDate={handleSelectDate}
              />
            )}
          </View>
        );
      }}
    />
  );
}
