...
import { useExpensesQuery, useExpenseTypesQuery } from "@/hooks/useExpenseQueries";
...
export default function ExpensesScreen() {
  ...
  const { data: typesResponse, isLoading: isTypesLoading } = useExpenseTypesQuery(
    token || "",
    restaurantId || "",
    1000,
  );
  ...
  const expenses = useMemo(() => {
    ...
    if (expensesResponse.data && Array.isArray(expensesResponse.data.data)) return expensesResponse.data.data;
    return [];
  }, [expensesResponse]);
  ...
  const expenseTypes = useMemo(() => {
    ...
    if (typesResponse.data && Array.isArray(typesResponse.data.data)) return typesResponse.data.data;
    return [];
  }, [typesResponse]);
  ...
  const activeFilterCount = useMemo(() => {
    ...
    if (filterValues.amount_range?.min || filterValues.amount_range?.max) count++;
  ...
  return (
    <SafeAreaView edges={["left", "right", "bottom"]} className="flex-1 bg-base-100">
      ...
      <RefreshableScrollView onRefresh={handleRefresh} contentContainerStyle={{ paddingBottom: 80 }}>
        {/*  Page Title */}
        <PageTitle title="Expenses" icon="receipt-long" badgeCount={expenses.length} />
        ...
        <View className="mt-2">
          {isLoading ? (
            <View className="py-20 items-center justify-center">
              ...
              <Text className="mt-3 text-xs font-semibold text-accent">Loading expenses...</Text>
            </View>
          ) : expenses.length > 0 ? (
