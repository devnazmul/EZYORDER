...
export default function ProfileScreen() {
  ...
  const { data, isLoading, refetch } = useOwnerProfileQuery(token || "", userId || null);
  ...
  const initials = useMemo(() => {
    ...
    const first = profileUser.first_Name ? profileUser.first_Name.charAt(0) : "";
  ...
  const renderContent = () => {
    ...
    const hasAddressGroup = !!(profileUser.door_no || profileUser.Address || profileUser.post_code);
      ...
      <View key="loaded" className="gap-y-6">
        ...
        <View className="bg-base-300 border border-base-200 rounded-xl p-5 shadow-sm items-center gap-3">
          <View className="w-24 h-24 rounded-full bg-primary/10 items-center justify-center border-4 border-base-200 shadow-sm overflow-hidden">
            {profileUser.image ? (
              <Image source={{ uri: profileUser.image }} className="w-full h-full" resizeMode="cover" />
            ) : (
              <Text className="text-3xl font-black text-primary">{initials}</Text>
            )}
          </View>
          <View className="items-center">
            <Text className="text-lg font-black text-neutral text-center">{fullName}</Text>
        ...
        <View className="bg-base-300 border border-base-200 rounded-xl px-4 py-1 shadow-sm">
          ...
          {renderDetailItem("restaurant", "Waiter Status", formatStatus(profileUser.waiter_status))}
          {renderDetailItem("local-shipping", "Driver Status", formatStatus(profileUser.driver_status))}
          {renderDetailItem("credit-card", "Stripe Account ID", profileUser.stripe_id || "Not Configured")}
