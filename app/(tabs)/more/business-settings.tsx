...
import { useExpenseTypesQuery } from "@/hooks/useExpenseQueries";
import { useBusinessTimingQuery, useRestaurantQuery } from "@/hooks/useRestaurantQueries";
...
export default function BusinessSettingsScreen() {
  ...
  const expenseTypesList = useMemo(() => {
    ...
    if (expenseTypesData.data && Array.isArray(expenseTypesData.data.data)) return expenseTypesData.data.data;
    return [];
  }, [expenseTypesData]);
  ...
  const renderTabContent = () => {
    ...
    if (activeTab === "info") {
      if (!restaurantData) {
        return (
          <View key="empty-info" className="flex-1 justify-center items-center py-10 bg-base-100">
            <EmptyState
              icon="error-outline"
      ...
      return (
        <View key="info-content" className="flex-1">
          <BusinessInfoCard settings={restaurantData?.restaurant || restaurantData} />
        </View>
      );
    }
    ...
    if (activeTab === "schedule") {
      if (timingList.length === 0) {
        return (
          <View key="empty-schedule" className="flex-1 justify-center items-center py-10 bg-base-100">
            <EmptyState
              icon="schedule"
      ...
      return (
        <View key="schedule-content" className="flex-1">
          {timingList.map((dayTiming: any, index: number) => (
            <BusinessScheduleCard key={dayTiming.day ?? index} timing={dayTiming} />
          ))}
        </View>
      );
    }
    ...
    if (activeTab === "expense_types") {
      if (expenseTypesList.length === 0) {
        return (
          <View key="empty-expenses" className="flex-1 justify-center items-center py-10 bg-base-100">
            <EmptyState
              icon="receipt"
