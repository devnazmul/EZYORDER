...
export default function UserDetailModal({ visible, onClose, user }: UserDetailModalProps) {
  ...
  return (
    <Modal visible={visible} transparent animationType="fade" onRequestClose={onClose}>
      <View className="flex-1 justify-center items-center bg-neutral/55 px-5">
        <View className="bg-base-300 border border-base-200 rounded-3xl w-full max-w-[340px] p-6 shadow-2xl relative overflow-hidden">
          
          {/* Header Close Button */}
          <TouchableOpacity onPress={onClose} className="absolute top-4 right-4 p-1 rounded-full bg-base-100 active:bg-base-200">
          ...
          <View className="pt-4 border-t border-base-200/50 flex-row gap-3">
            <View className={`w-10 h-10 rounded-xl ${roleStyles.bg} items-center justify-center flex-shrink-0`}>
              <MaterialIcons name="verified-user" size={20} color={roleStyles.text === "text-primary" ? "#DC2D2A" : roleStyles.text === "text-secondary" ? "#00677F" : "#6E6E6E"} />
            </View>
          ...
          <TouchableOpacity
            onPress={onClose}
            className="w-full bg-primary py-3 rounded-xl items-center mt-6 active:opacity-90"
          >
